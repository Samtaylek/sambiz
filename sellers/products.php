...
//        $list_price = ((isset($_POST['list_price']))?sanitize($_POST['list_price']):$product['list_price']);
    ...
    if($_POST) {
        $errors = array();
        $required = array('title','price','sizes','address');
        ...
        if (!empty($errors)) {
            ...
            $insertSql = "INSERT INTO custom_goods (`title`,`price`,`sizes`,`image`,`description`,`address`)
 VALUES ('$title','$price','$sizes','$dbPath','$description','$address') ";
            if(isset($_GET['edit'])){
                $insertSql = "UPDATE custom_goods SET title = '$title', price = '$price', sizes = '$sizes', image = '$dbPath', description = '$description', address = '$address'
  WHERE id = '$edit_id'";
            }
            $db->query($insertSql);
            if (mysqli_error($db) != ''){
    ...
    <form action="products.php?<?= ((isset($_GET['edit']))?'edit='.$edit_id:'add=1');?>" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group col-md-3">
            ...
            <!--            <button  class="btn btn-default form-control" type="button" data-toggle="modal" data-target="#sizesModal">Quantity & sizes</button>-->
            <button type="button" class="btn btn-primary form-control" onclick="$('#sizesModal').modal('toggle'); return false;" >Quantity & Sizes</button>
        </div>
        <div class="form-group col-md-3">
